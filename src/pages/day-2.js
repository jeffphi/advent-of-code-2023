...
export default function Day2() {
    ...
    function solve_part1(){
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if(isGameValid(data[i])){
                // Note that input row[0] is Game 1
                retVal += i+1;
            }
        }
    ...
    function isGameValid(line){
        ...
        for(let i = 0; i < turnsArray.length; i++){
            ...
            for (const set of sets){
                ...
                if (vals[0]>12 || vals[1]>13 || vals[2]>14){
                    return false;
                }
            }
        }
    ...
    // A set looks like: "3 green, 4 blue, 1 red"
    function getRGBFromSet(set){
        let r = 0
        let g = 0;
        let b = 0;
        ...
        for (const draw of draws){
            let parts = draw.trim().split(" ");
            if(parts[1].includes("red")){
                r += parseInt(parts[0]);
            } else if (parts[1].includes("green")){
                g += parseInt(parts[0]);
            } else {
                b += parseInt(parts[0]);
            }

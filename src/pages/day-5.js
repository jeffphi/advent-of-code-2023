...
export default function Day5() {
    ...
    function solve_Test(){
        ...
        for(const seed of seedsTest){
            
            let soil = getNext(seed,seedToSoilTest);
            let fertilizer = getNext(soil,soilToFertilizerTest);
            let water = getNext(fertilizer, fertilizerToWaterTest);
            let light = getNext(water, WaterToLightTest);
            let temperature = getNext(light, lightToTemperatureTest);
            let humidity = getNext(temperature, temperatureToHumidityTest);
            let location = getNext(humidity,humidityToLocationTest);
            console.log("Test Seed "+seed+', soil '+soil+', fert '+fertilizer+', water '+water+', light '+light+', temp '+temperature+', humid '+humidity+', location '+location);

            if (location < retVal){
                console.log("New lowest Test location: "+location);
                retVal = location;
            }
        }

        console.log('Test answer: ' + retVal);
    }

    function getNext(key, map){
        for(const entry of map){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            console.log('Key '+key+', Dest '+dest+', Source '+source+', Range '+range);
            if((key >= source) && (key <= (source+range-1))){
                console.log('Returning hit: '+(dest + key - source));
    ...
    function getFertilizer(soil){
        for(const entry of soilToFertilizer){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(soil >= source && soil <= source+range-1){
                return dest + soil - source;
            }
        }
    ...
    function getWater(fertilizer){
        for(const entry of fertilizerToWater){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(fertilizer >= source && fertilizer <= source+range-1){
                return dest + fertilizer - source;
            }
        }
    ...
    function getLight(water){
        for(const entry of WaterToLight){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(water >= source && water <= source+range-1){
                return dest + water - source;
            }
        }
    ...
    function getTemperature(light){
        for(const entry of lightToTemperature){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(light >= source && light <= source+range-1){
                return dest + light - source;
            }
        }
    ...
    function getHumidity(temperature){
        for(const entry of temperatureToHumidity){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(temperature >= source && temperature <= source+range-1){
                return dest + temperature - source;
            }
        }
    ...
    function getLocation(humidity){
        for(const entry of humidityToLocation){
            let source = entry[1];
            let range = entry[2];
            let dest = entry[0];
            if(humidity >= source && humidity <= source+range-1){
                return dest + humidity - source;
            }
        }
    ...
    const input = [];
    input.push(<div>Seeds:</div>);
    for(const seed of seeds){
        input.push(<div>{seed}</div>);
    }

    input.push(<div>Seed to Soil Map:</div>);
    for(const items of seedToSoil){
        input.push(<div>{items[0]},{items[1]},{items[2]}</div>);
    }
...
const seedToSoilTest = [
    [50, 98, 2],
    [52, 50, 48]
];

const soilToFertilizerTest = [
    [0,15,37],
    [37,52,2],
    [39,0,15]
];

const fertilizerToWaterTest = [
    [49, 53, 8],
    [0, 11, 42],
    [42,0 ,7],
    [57, 7, 4]
...
const WaterToLightTest = [
    [88, 18, 7],
    [18, 25, 70]
];

const lightToTemperatureTest = [
    [45, 77, 23],
    [81, 45, 19],
    [68, 64, 13],
];

const temperatureToHumidityTest = [
    [0, 69, 1],
    [1, 0, 69]
];

const humidityToLocationTest = [
    [60, 56, 37],
    [56, 93, 4]
];

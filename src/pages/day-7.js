...
export default function Day7() {
    function solve_part1(){
        ...
        for(let i=0; i<hands.length; i++){
            console.log('Bid '+hands[i].bid+', Rank '+ (i+1));
            retVal += (hands[i].bid*(i+1));
        }
    ...
    function rankHand(hand){
        ...
        // 2 pair       16
        // 1 pair       15
        // high card    14 (will be figured out by tie-breaking rules later)

        let cardMap = new Map();
        for(let i=0; i<5; i++){
            if(cardMap.has(hand[i])){
                cardMap.set(hand[i],cardMap.get(hand[i])+1);
            } else{
                cardMap.set(hand[i],1);
            }
        }
        console.log(cardMap);
    ...
    function compareHands(a, b) {
        ...
        // Ranks are the same, need to compare cards
        for(let i=0; i<5; i++){
            let cardA = parseInt(a.cards[i]);
            let cardB =parseInt(b.cards[i]);

            if(isNaN(cardA)){
                cardA = cardLetterToVal(a.cards[i]);
            }
            if(isNaN(cardB)){
                cardB = cardLetterToVal(b.cards[i]);
            }
            ...
            //console.log('Card A: '+cardA+', Carb B: '+cardB);
        }

        // Actually equal, should not get here...
    ...
    function cardLetterToVal(letter){
        switch (letter){
            case 'A':
                return 14;
            case 'K':
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        input.push(<div key={i}>{spans} </div>);
    }
